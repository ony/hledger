...
bucketsByOpts CliOpts{rawopts_ = rawopts} j'
        | not $ boolopt "budget-buckets" rawopts = j'
        | null buckets = j'
...
offsetByOpts opts@CliOpts{rawopts_ = rawopts} j = journalBalanceTransactions' opts j{ jtxns = ts' }
    ...
    where
        ts = jtxns j
        dates = jdatespan j
        ts' | boolopt "budget-offset" rawopts = [makeBudget t | pt <- jperiodictxns j, t <- runPeriodicTransaction pt dates] ++ ts
            | otherwise = ts
...
budgetWrapper f opts = f opts . budgetByOpts opts . journalBalanceTransactions' opts
...
main = do
    ...
    withJournalDo (assumeBudgetCliOpts opts) budget

assumeBudgetCliOpts :: CliOpts -> CliOpts
assumeBudgetCliOpts opts@CliOpts{rawopts_ = raw} = opts {rawopts_ = addOffsetOpt . addBucketsOpt $ raw} where
    addBucketsOpt = if boolopt "no-buckets" raw then id else (("budget-buckets", ""):)
    addOffsetOpt = if boolopt "no-offset" raw then id else (("budget-offset", ""):)
